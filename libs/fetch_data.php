...
function getgalleryposts($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE posted='publish' ORDER BY id DESC LIMIT 3";
	if ($result=mysqli_query($con,$sql))
	{
...
function getblogridposts($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE posted='publish' ORDER BY id DESC LIMIT 12";
	if ($result=mysqli_query($con,$sql))
	{
      	//count number of rows in query result
		$rowcount=mysqli_num_rows($result);
...
function geteventposts($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		if ($rowcount==0) {
      	...
      	//if there are rows available display all the results
		foreach ($result as $events => $ev) {
      	# code...
			echo '<div class="blog-grids row mb-3">
			<div class="col-md-5 blog-grid-left">
			<a href="single.php?id='.$ev['id'].'">
			<img src="blogadmin/images/'.$ev['photo'].'" class="img-fluid" alt="fantastic cms">
			</a>
			</div>
			<div class="col-md-7 blog-grid-right">

			<h5>
				<a href="single.php?id='.$ev['id'].'">'.$ev['title'].'</a>
			</h5>
			<div class="sub-meta">
			<span>
			<i class="far fa-clock"></i> '.$ev['date'].'</span>
			</div>
			</div>
...
function getmagazinesposts($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE posted='publish' ORDER BY id ASC LIMIT 8";
	if ($result=mysqli_query($con,$sql))
	{
		...
		if ($rowcount==0) {
      	...
      	//if there are rows available display all the results
		foreach ($result as $magazines => $mg) {
      	# code...
			echo '<div class="blog-grids row mb-3">
			<div class="col-md-5 blog-grid-left">
			<a href="single.php?id='.$mg['id'].'">
			<img src="blogadmin/images/'.$mg['photo'].'" class="img-fluid" alt="fantastic cms">
			</a>
			</div>
			<div class="col-md-7 blog-grid-right">

			<h5>
				<a href="single.php?id='.$mg['id'].'">'.$mg['title'].'</a>
			</h5>
			<div class="sub-meta">
			<span>
			<i class="far fa-clock"></i> '.$mg['date'].'</span>
			</div>
			</div>
...
function getaboutdescription($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table ";
	if ($result=mysqli_query($con,$sql))
	{
