...
// Data functions (insert, update, delete, form) for table soft_skill

// This script and data application were generated by AppGini 5.70
// Download AppGini for free from https://bigprof.com/appgini/download/

function soft_skill_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('soft_skill');
	if(!$arrPerm[1]){
		return false;
	}
...
function soft_skill_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('soft_skill');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='soft_skill' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='soft_skill' and pkValue='$selected_id'");
	...
	// hook: soft_skill_before_delete
	if(function_exists('soft_skill_before_delete')){
		$args=array();
		if(!soft_skill_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `soft_skill` where `id`='$selected_id'", $eo);

	// hook: soft_skill_after_delete
	if(function_exists('soft_skill_after_delete')){
		$args=array();
		soft_skill_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='soft_skill' and pkValue='$selected_id'", $eo);
}

function soft_skill_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('soft_skill');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='soft_skill' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='soft_skill' and pkValue='".makeSafe($selected_id)."'");
...
function soft_skill_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `soft_skill` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'soft_skill_view.php', false);
		}
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/soft_skill_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/soft_skill_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return soft_skill_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return soft_skill_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return soft_skill_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('soft_skill');
	if($selected_id){
		$jdata = get_joined_record('soft_skill', $selected_id);
		if($jdata === false) $jdata = get_defaults('soft_skill');
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('soft_skill-ajax-cache', $cache_data);

	// hook: soft_skill_dv
	if(function_exists('soft_skill_dv')){
		$args=array();
		soft_skill_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
