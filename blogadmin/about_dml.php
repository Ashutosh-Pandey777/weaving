...
// Data functions (insert, update, delete, form) for table about
...
function about_insert(){
	...
	$arrPerm=getTablePermissions('about');
	if(!$arrPerm[1]){
		return false;
	}
	...
	if($data['icon']) createThumbnail($data['icon'], getThumbnailSpecs('about', 'icon', 'tv'));
	if($data['icon']) createThumbnail($data['icon'], getThumbnailSpecs('about', 'icon', 'dv'));

	/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
	if($_REQUEST['SelectedID']){
		$res = sql("select * from about where id='" . makeSafe($_REQUEST['SelectedID']) . "'", $eo);
		if($row = db_fetch_assoc($res)){
			if(!$data['icon']) $data['icon'] = makeSafe($row['icon']);
		}
	}

	// hook: about_before_insert
	if(function_exists('about_before_insert')){
		$args=array();
		if(!about_before_insert($data, getMemberInfo(), $args)){ return false; }
	}
	...
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"about_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: about_after_insert
	if(function_exists('about_after_insert')){
		$res = sql("select * from `about` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!about_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('about', $recID, getLoggedMemberID());

	return $recID;
}

function about_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('about');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='about' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='about' and pkValue='$selected_id'");
	...
	// hook: about_before_delete
	if(function_exists('about_before_delete')){
		$args=array();
		if(!about_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `about` where `id`='$selected_id'", $eo);

	// hook: about_after_delete
	if(function_exists('about_after_delete')){
		$args=array();
		about_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='about' and pkValue='$selected_id'", $eo);
}

function about_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('about');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='about' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='about' and pkValue='".makeSafe($selected_id)."'");
...
function about_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `about` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'about_view.php', false);
		}
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/about_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/about_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return about_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return about_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return about_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('about');
	if($selected_id){
		$jdata = get_joined_record('about', $selected_id);
		if($jdata === false) $jdata = get_defaults('about');
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('about-ajax-cache', $cache_data);

	// hook: about_dv
	if(function_exists('about_dv')){
		$args=array();
		about_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
