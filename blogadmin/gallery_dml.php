...
// Data functions (insert, update, delete, form) for table gallery
...
function gallery_insert(){
	...
	$arrPerm=getTablePermissions('gallery');
	if(!$arrPerm[1]){
		return false;
	}
	...
	if($data['photo']) createThumbnail($data['photo'], getThumbnailSpecs('gallery', 'photo', 'tv'));
	if($data['photo']) createThumbnail($data['photo'], getThumbnailSpecs('gallery', 'photo', 'dv'));
	if($data['title']== ''){
	...
	if($_REQUEST['SelectedID']){
		$res = sql("select * from gallery where id='" . makeSafe($_REQUEST['SelectedID']) . "'", $eo);
		if($row = db_fetch_assoc($res)){
			if(!$data['photo']) $data['photo'] = makeSafe($row['photo']);
		}
	}

	// hook: gallery_before_insert
	if(function_exists('gallery_before_insert')){
		$args=array();
		if(!gallery_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `gallery` set       `title`=' . (($data['title'] !== '' && $data['title'] !== NULL) ? "'{$data['title']}'" : 'NULL') . ', `category`=' . (($data['category'] !== '' && $data['category'] !== NULL) ? "'{$data['category']}'" : 'NULL') . ', `tags`=' . (($data['tags'] !== '' && $data['tags'] !== NULL) ? "'{$data['tags']}'" : 'NULL') . ', `content`=' . (($data['content'] !== '' && $data['content'] !== NULL) ? "'{$data['content']}'" : 'NULL') . ', ' . ($data['photo'] != '' ? "`photo`='{$data['photo']}'" : '`photo`=NULL') . ', `date`=' . "'{$data['date']}'" . ', `author`=' . "'{$data['author']}'" . ', `posted`=' . (($data['posted'] !== '' && $data['posted'] !== NULL) ? "'{$data['posted']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"gallery_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: gallery_after_insert
	if(function_exists('gallery_after_insert')){
		$res = sql("select * from `gallery` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!gallery_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('gallery', $recID, getLoggedMemberID());

	return $recID;
}

function gallery_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('gallery');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='gallery' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='gallery' and pkValue='$selected_id'");
	...
	// hook: gallery_before_delete
	if(function_exists('gallery_before_delete')){
		$args=array();
		if(!gallery_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// child table: editors_choice
	$res = sql("select `id` from `gallery` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "editors_choice", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='gallery_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='gallery_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}

	sql("delete from `gallery` where `id`='$selected_id'", $eo);

	// hook: gallery_after_delete
	if(function_exists('gallery_after_delete')){
		$args=array();
		gallery_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='gallery' and pkValue='$selected_id'", $eo);
}

function gallery_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('gallery');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='gallery' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='gallery' and pkValue='".makeSafe($selected_id)."'");
	...
	$data['photo'] = PrepareUploadedFile('photo', 5120000, 'jpg|jpeg|gif|png', false, "");
	$existing_photo = sqlValue("select `photo` from `gallery` where `id`='" . makeSafe($selected_id) . "'");
	...
	// hook: gallery_before_update
	if(function_exists('gallery_before_update')){
		$args=array();
		if(!gallery_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `gallery` set       `title`=' . (($data['title'] !== '' && $data['title'] !== NULL) ? "'{$data['title']}'" : 'NULL') . ', `category`=' . (($data['category'] !== '' && $data['category'] !== NULL) ? "'{$data['category']}'" : 'NULL') . ', `tags`=' . (($data['tags'] !== '' && $data['tags'] !== NULL) ? "'{$data['tags']}'" : 'NULL') . ', `content`=' . (($data['content'] !== '' && $data['content'] !== NULL) ? "'{$data['content']}'" : 'NULL') . ', ' . ($data['photo']!='' ? "`photo`='{$data['photo']}'" : '`photo`=`photo`') . ', `date`=`date`' . ', `posted`=' . (($data['posted'] !== '' && $data['posted'] !== NULL) ? "'{$data['posted']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="gallery_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: gallery_after_update
	if(function_exists('gallery_after_update')){
		$res = sql("SELECT * FROM `gallery` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!gallery_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='gallery' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function gallery_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/gallery.posted.csv')){
		$posted_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/gallery.posted.csv')));
	...
	if($selected_id){
		...
		$res = sql("select * from `gallery` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'gallery_view.php', false);
		}
			...
			$j("#category-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_category__RAND__.value, t: 'gallery', f: 'category' },
						success: function(resp){
							...
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=gallery_categories_view_parent]').hide(); }else{ $j('.btn[id=gallery_categories_view_parent]').show(); }
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p: page, t: 'gallery', f: 'category' }; },
				...
				if(e.added.id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=gallery_categories_view_parent]').hide(); }else{ $j('.btn[id=gallery_categories_view_parent]').show(); }
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/gallery_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/gallery_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return gallery_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return gallery_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return gallery_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$lookup_fields = array(  'category' => array('gallery_categories', 'Category'));
	...
	$rdata = $jdata = get_defaults('gallery');
	if($selected_id){
		$jdata = get_joined_record('gallery', $selected_id);
		if($jdata === false) $jdata = get_defaults('gallery');
		$rdata = $row;
	}
	$cache_data = array(
	...
	$templateCode .= loadView('gallery-ajax-cache', $cache_data);

	// hook: gallery_dv
	if(function_exists('gallery_dv')){
		$args=array();
		gallery_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
